...
- **"Large payload received (302332 bytes), should be less than 100kb. Solution: transform runtime."** → TRMNL caps polling responses at 100 KB. The Capital Bikeshare GBFS feed exceeds that. Solution: paste [`plugins/transit/transform.js`](../plugins/transit/transform.js) into the transit plugin's Markup Editor → Transform tab. The transform filters the 300 KB GBFS feed down to just your configured stations (~5 KB) before the payload reaches the markup.
